...
public class MapScreen extends ScreenAdapter {
  ...
  private static final Color PAUSED_DULLING = new Color(0.5f, 0.5f, 0.5f, 1f);
  ...
  public MapScreen(final Main game) {
    ...
    VariantProperties[] jointTabVariants = new VariantProperties[CateringVariant.values().length
        + RecreationVariant.values().length];
    System.arraycopy(
        CateringVariant.values(),
        0,
        jointTabVariants,
        0,
        CateringVariant.values().length
    );
    System.arraycopy(
        RecreationVariant.values(),
        0,
        jointTabVariants,
        CateringVariant.values().length,
        RecreationVariant.values().length
    ...
    Gdx.input.setInputProcessor(new InputAdapter() {
      ...
      public boolean touchDown(int screenX, int screenY, int pointer, int button) {
        ...
        Vector3 screenPos = viewport.getCamera().unproject(
            new Vector3(screenX, screenY, 0), viewport.getScreenX(),
        ...
        if (menuItem >= 0 && menuItem < 5) {
          addObject(buildingVariants.get(menuTab)[menuItem],
              pixelToTile((int) (screenPos.x * camera.scale), (int) (screenPos.y * camera.scale)));
          menuItem = -1;
        } else if (menuItem == 5) {
          try {
            removeObject(
      ...
      public boolean mouseMoved(int x, int y) {
        screenPos = viewport.getCamera().unproject(
            new Vector3(x, y, 0), viewport.getScreenX(), viewport.getScreenY(),
  ...
  /**
   * Converts building tile coordinates to pixel coordinates. Must account for
   * camera.scale and building depth separately.
   *
  ...
  public int[] tileToPixel(GridPosition coords) {
    ...
    int pixelY = -1343 + (coords.getX() - coords.getY()) * 16 + camera.y 
        + (int) (HEIGHT * camera.scale);
  ...
  /**
   * Draw a building onto the game screen.
   *
  ...
  public void render(float delta) {
    ...
    game.spritebatch.draw(gameMap[1], 0, 0, WIDTH, HEIGHT, camera.x - gameMap[0].getWidth() + 3, 
        camera.y + 1, (int) (WIDTH * camera.scale), (int) (HEIGHT * camera.scale), false, false);
    game.spritebatch.draw(gameMap[2], 0, 0, WIDTH, HEIGHT, camera.x + 1,
        camera.y - gameMap[0].getHeight() + 3, (int) (WIDTH * camera.scale),
        (int) (HEIGHT * camera.scale), false, false
    );
    game.spritebatch.draw(gameMap[3], 0, 0, WIDTH, HEIGHT, camera.x - gameMap[0].getWidth() + 3,
        camera.y - gameMap[0].getHeight() + 3,
        (int) (WIDTH * camera.scale), (int) (HEIGHT * camera.scale), false, false);
    ...
    for (int i = 0; i < 6; i++) {
      ...
      game.spritebatch.draw(
          ...
          15,
          50,
          (int) ((float) buildingTextures[j].getHeight() / buildingTextures[j].getWidth() * 50),
          0,
          0,
          buildingTextures[j].getWidth(),
          buildingTextures[j].getHeight(),
          false,
          false
      );
    }
